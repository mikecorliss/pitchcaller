      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar">
        ...
        <section className="p-5 border-b border-slate-100">
          ...
          <div className="space-y-5">
            ...
            {/* Wristband Size */}
            <div>
              <label className="block text-[11px] font-bold text-slate-700 uppercase mb-2">Wristband Size</label>
              <div className="grid grid-cols-2 gap-4">
                <div>
                   <label className="block text-xs font-semibold text-slate-500 mb-1.5">Width (in)</label>
                   <input 
                     type="number" 
                     step="0.05"
                ...
                <div>
                   <label className="block text-xs font-semibold text-slate-500 mb-1.5">Height (in)</label>
                   <input 
                     type="number" 
                     step="0.05"
            ...
            <div className="pt-2 border-t border-slate-100">
              <label className="block text-[11px] font-bold text-slate-700 uppercase mb-2">Coach Key Size</label>
              <div className="grid grid-cols-2 gap-4">
                <div>
                   <label className="block text-xs font-semibold text-slate-500 mb-1.5">Width (in)</label>
                   <input 
                     type="number" 
                     step="0.05"
                     value={config.coachPrintWidth}
                     onChange={(e) => setConfig({ ...config, coachPrintWidth: Number(e.target.value) })}
                     className="w-full bg-slate-50 border border-slate-200 rounded-md px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500 outline-none"
                   />
                </div>
                <div>
                   ...
                   <input 
                     type="number" 
                     step="0.05"
                     value={config.coachPrintHeight}
                     onChange={(e) => setConfig({ ...config, coachPrintHeight: Number(e.target.value) })}
                     className="w-full bg-slate-50 border border-slate-200 rounded-md px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500 outline-none"
                   />
                </div>
              </div>
